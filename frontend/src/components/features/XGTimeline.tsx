...
const MOCK_SHOTS = Array.from({ length: 24 }, (_, i) => ({
  minute: i * 4 + Math.floor(Math.random() * 3),
  xg: parseFloat((Math.random() * 0.6).toFixed(2)),
  team: Math.random() > 0.5 ? 'home' : 'away',
  isGoal: Math.random() > 0.88,
}));
  ...
  return (
    <div className="card p-4">
      ...
      <ResponsiveContainer width="100%" height={140}>
        <BarChart data={MOCK_SHOTS} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
